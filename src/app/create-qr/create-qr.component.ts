import { Component, OnInit } from '@angular/core';
import { ImportService } from '../service/import.service';
import { GenerateQRService } from '../service/generate-qr.service';
import { formatDate } from '@angular/common';
import Swal from 'sweetalert2';
declare var $: any;

@Component({
  selector: 'app-create-qr',
  templateUrl: './create-qr.component.html',
  styleUrls: ['./create-qr.component.css']
})
export class CreateQRComponent implements OnInit {
  datas: any;
  datasy:any;
  invoice: any;
  quantity: any;
  qty: any = 0;
  sum: any = 0;
  l_model: any;
  l_qty: any;
  p_model: any;
  p_qty: any;
  data_logis: any = [];
  data_production: any = [];
  get_inv: any = [];
  studentOptions: any;
  studentOptions2: any;
  arr: any = [];
  type: any;
  count_pallet: any = [];

  // print
  qrInfo: any = [];
  count_data: any = [];
  body: any = [];
  imageSource: any;
  data: any = [];
  studentOptions3: any;
  data2: any;
  item:any;
  story: any;
  x = 1
  y = 0
  checked : any = 'A';
  checked_num : any = [];
  
  constructor(private import_: ImportService, private gen_qr: GenerateQRService) { }

  ngOnInit(): void {
    this.get_data()
    this.type = 'Normal'
    this.imageSource = 'data:image/jpeg;base64,';
  }

  get_data() {
    let arr: any = []
    let today = formatDate(new Date(), 'yyyy-MM-dd', 'en-US')
    this.import_.get_import_bydate().subscribe((data: any) => { //data logis
      this.story = data[0].story
      if (data) {
        this.datas = data
        // distinct data
        this.gen_qr.get_qrcode_bydate().subscribe((datas: any) => { //gen_qr.get_qrcode_bydate() // data production
          console.log("this.datas",this.datas);
                    
          this.datasy = datas
          this.studentOptions2 = datas.filter((item: any, i: any, arr: any) => arr.findIndex((t: any) => t.invoice === item.invoice) === i);        //data gen qr 
          this.studentOptions = this.datas.filter((item: any, i: any, arr: any) => arr.findIndex((t: any) => t['Delivery Note#'] === item['Delivery Note#']) === i);      // data all logis   
          console.log("this.studentOptions2",this.studentOptions2);
          
          for (let j = 0; j < this.studentOptions.length; j++) {
            if(this.studentOptions[j].select_inv != '1'){
              this.get_inv.push(this.studentOptions[j]['Delivery Note#']);
            }
          }
          // let getcount = []
          // this.get_inv = []
          // for (let k = 0; k < this.studentOptions2.length; k++) {
          //   for (let j = 0; j < this.studentOptions.length; j++) {
          //     // if (this.studentOptions[j]['Delivery Note#'] === this.studentOptions2[k].invoice) {
          //     //   getcount.push(j)
          //     // }
          //     if (this.studentOptions2[k].invoice != this.studentOptions[j]['Delivery Note#']) {
          //       getcount.push(this.studentOptions[j]['Delivery Note#'])
          //       // this.get_inv.push(this.studentOptions[j]['Delivery Note#'])
          //     }
          //   }
          // }          
          console.log("this.get_inv",this.get_inv);
          
          // console.log("A",this.get_inv);
          
          // let i = 0
          // for (i = getcount.length; i > 0;) {
          //   i -= 1
          //   this.get_inv.splice(getcount[i], 1)
          // }
        });
      }
    });
  }

  addscan(){
    let A = this.arr.length
    this.arr.push(this.arr.length + 1)    
    for (let i = A; i < this.arr.length; i++) {
      let body = {
        p_model: '',
        p_qty: Number()
      }
      this.data_production[i] = body
    }
  }

  remove(i:any){
    this.qty = Number(this.qty) - Number(this.data_production[i].p_qty)
    this.arr.splice(i,1)
    this.data_production.splice(i,1)
  }

  get_invoice() {
    this.data_logis = []
    this.arr = [1, 2, 3, 4, 5]
    this.sum = 0
    let body = {
      invoice: this.invoice
    }
    for (let i = 0; i < 5; i++) {
      let body = {
        p_model: '',
        p_qty: Number('')
      }
      this.data_production[i] = body
      
    }    
    this.import_.get_import_invoice(body).subscribe((data: any) => {
      if (data) {
        for (let i = 0; i < data.length; i++) {
          if (data.length > 0) {
            let sales = Number(data[i]['Sales QTY'])
            let A = data[i]['Customer Part#'].split('-')
            this.l_model = A[0] + '-' + A[1]
            this.l_qty = data[i]['Sales QTY']
            this.sum += sales
          } else {
            this.sum = Number(data[i]['Sales QTY'])
            let A = data[i]['Customer Part#'].split('-')
            this.l_model = A[0] + '-' + A[1]
            this.l_qty = data[i]['Sales QTY']
            // this.data_production[i] = { p_model: ' ', p_qty: Number(' ') }
            // this.data_logis[this.arr] = {model : this.l_model,qty : this.l_qty}            
          }
          this.quantity = this.sum
        }
        this.test_()
      }
    });
    
    
    // model.focus()
  }

  test_(){
    if(this.quantity != undefined){
      setTimeout(() => {
        let A:any = document.getElementById("test"+0)
        A.focus();
      },500);      
    }
  }

  _checked(i:any){
    this.checked_num.push(i)
  }

  check_qrcode(i: any) {
    let A = this.invoice.split('')    
    this.checked = A    
    this.qty = 0    
    if (this.type == 'Special') {
      if(this.checked_num == ''){
        this.p_model = this.data_production[i]?.p_model.slice(6, 17);
        this.p_qty = Number(this.data_production[i]?.p_model.slice(18, 22));
      }else{
        // for (let j = 0; j < this.checked_num.length; j++) {
          this.p_model = this.data_production[this.checked_num]?.p_model;
          this.p_qty = 0;
        // }
        this.checked_num = []
      }
    } else {
      if(A[8] == 'C'){
        this.p_model = this.data_production[i]?.p_model;
        this.p_qty = 0;
      }else{
        if (this.data_production[i]?.p_model.length == 23) {
          this.p_model = this.data_production[i]?.p_model.slice(7, 18);
          this.p_qty = Number(this.data_production[i]?.p_model.slice(19, 23));
        } else {
          this.p_model = this.data_production[i]?.p_model.slice(8, 19);
          this.p_qty = Number(this.data_production[i]?.p_model.slice(20, 24));
        }
      }
    }

      this.data_production[i].p_model = this.p_model
      this.data_production[i].p_qty = this.p_qty
      for (let i = 0; i < this.arr.length; i++) {
        this.qty += this.data_production[i].p_qty
      }
     this._test(i)
  }

  _test(i:any){
    let n = i+1
    if(this.data_production[i].p_qty != 0){
      setTimeout(() => {
        let A:any = document.getElementById("test"+n)
        A.focus();
      },500);
    }
  }

  clear(){
    for (let i = 0; i < this.arr.length; i++) {
      this.data_production[i].p_model = []
      this.data_production[i].p_qty = []
    }
    this.qty = 0
    this.test_()
  }

  sum_qty(arr: any){
    this.qty = 0
    for (let p = 0; p < arr.length; p++) {
      for (let j = this.y; j < this.x; j++) {
        if(this.y == 0){
          this.qty = 0
        }
        this.p_qty = Number(this.data_production[p].p_qty)
        this.qty += Number(this.p_qty)
      }
      this.x = this.x+1
      this.y = this.y+1
    }
  }

  save_qr(status: any) {
    let x = 0 // No_pallet
    let y = 0 // Total_pallet
    let date = new Date()
    for (let i = 0; i < this.data_production.length; i++) {
      if (this.data_production[i].p_qty != 0) {
      // if (this.data_production[i].p_model != '' && this.data_production[i].p_qty != 0) {
        y += 1     // Total_pallet
      }
    }
    let B = []
    let body2
    for (let i = 0; i < this.data_production.length; i++) {
      if ( this.data_production[i].p_qty != 0) {
      // if (this.data_production[i].p_model != '' && this.data_production[i].p_qty != 0) {
        x += 1     // No_pallet
        let body = {
          model: this.data_production[i].p_model,
          qty: this.data_production[i].p_qty,
          invoice: this.invoice,
          no_pallet: x,
          total_pallet: y,
          type: this.type,
          status: '0',
          date: date,
          sendmail : 'false',
          story : this.story
        }     
        B.push(body)   
        body2 = {
          select_inv : '1',
          invoice: this.invoice,
        }
          if (status != 'save') {
            B[i].status = '1'            
          }
      }
    }
    this.import_.update_import(body2).subscribe((data: any) => {});
    if (status == 'save') {
      this.gen_qr.create_qrcode(B).subscribe((data: any) => {
      Swal.fire({
          icon: 'success',
          title: 'Complete!!',
          showConfirmButton: false,
          timer: 1500,
        }).then(function () {
          window.location.reload()
        });
      });
    } else {
      this.gen_qr.create_qrcode(B).subscribe((datas: any) => {
        this.gen_qr.get_qrcode_bydate().subscribe((data: any) => {
                        if(data){
                          this.datasy = data
                          this.studentOptions3 = this.datasy.filter((item: any, i: any, arr: any) => arr.findIndex((t: any) => t.invoice === item.invoice ) === i);
                          console.log("this.studentOptions3",this.studentOptions3);
                          
                          for (let x = 0; x < this.studentOptions3.length; x++) {
                            for (let i = 0; i < this.datasy.length; i++) {
                              if(this.datasy[i].invoice == this.studentOptions3[x].invoice && this.datasy[i].model == this.studentOptions3[x].model ){
                                this.sum += Number(this.datasy[i].qty)      
                              }
                              
                              // if(data[i].invoice == this.invoice){                   
                              //   this.data.push({ invoice: data[i].invoice, model: data[i].model, total_pallet: data[i].total_pallet, qty: this.sum, status: data[i].status })
                              // }
                            
                            this.data.push({invoice : this.studentOptions3[x].invoice,model : this.studentOptions3[x].model,total_pallet : this.studentOptions3[x].total_pallet,qty : this.sum,status : this.studentOptions3[x].status})
                            }
                            this.sum = 0
                            console.log("this.data",this.data);
                          }
                          
                          for (let p = 0; p < this.data.length; p++) {
                            if(this.data[p].invoice == this.invoice){
                              this.item = this.data[p]
                            }
                          }     
                            this.print_label(this.item)
                        }
                      });
      });
    }
  }

  print_label(item:any){    
    this.count_data = []  
    for (let i = 0; i < this.datasy.length; i++) {        
      if(this.type == 'Normal'){        
        if(item.model == this.datasy[i].model && item.invoice == this.datasy[i].invoice){
          this.count_data.push(this.datasy[i])   
          let bu =    this.datasy[i].invoice + "," +  this.datasy[i].qty + "," +  this.datasy[i].no_pallet + ',' +  this.datasy[i].total_pallet
          this.body.push(bu)
        } 
      }else{
        if(item.invoice == this.datasy[i].invoice){
          this.count_data.push(this.datasy[i])   
          let bu =    this.datasy[i].invoice + "," +  this.datasy[i].qty + "," +  this.datasy[i].no_pallet + ',' +  this.datasy[i].total_pallet
          this.body.push(bu)
        } 
      }
    }
    for (let k = 0; k < this.count_data.length; k++) {
      this.count_data[k].qrInfo = this.body[k]    
    }    
    this.count(item);    
  }

  count(item: any) {
    let A = item.status = 1
    let body = {
      invoice: item.invoice,
      model: item.model,
      status: A,
    }
    this.gen_qr.update_qrcode(body).subscribe((data: any) => {
      this.type = 1
      this.print();
    })
  }

  print() {
    const printContent: any = document.getElementById("print");
    const WindowPrt: any = window.open('', '', 'left=0,top=0,width=900,height=900,toolbar=0,scrollbars=0,status=0');
    WindowPrt.document.write(printContent.innerHTML);
    WindowPrt.document.close();
    WindowPrt.focus();
    WindowPrt.print();
    WindowPrt.close();
    this.type = 0
    window.location.reload()
  }

  // save_qr(status: any) {
  //   let x = 0 // No_pallet
  //   let y = 0 // Total_pallet
  //   let date = new Date()
  //   for (let i = 0; i < this.data_production.length; i++) {
  //     if (this.data_production[i].p_qty != 0) {
  //     // if (this.data_production[i].p_model != '' && this.data_production[i].p_qty != 0) {
  //       y += 1     // Total_pallet
  //     }
  //   }
  //   for (let i = 0; i < this.data_production.length; i++) {
  //     if ( this.data_production[i].p_qty != 0) {
  //     // if (this.data_production[i].p_model != '' && this.data_production[i].p_qty != 0) {
  //       x += 1     // No_pallet
  //       let body = {
  //         model: this.data_production[i].p_model,
  //         qty: this.data_production[i].p_qty,
  //         invoice: this.invoice,
  //         no_pallet: x,
  //         total_pallet: y,
  //         type: this.type,
  //         status: '0',
  //         date: date,
  //         sendmail : 'false',
  //         story : this.story
  //       }        
  //       let body2 = {
  //         select_inv : '1',
  //         invoice: this.invoice,
  //       }
  //         if (status == 'save') {
  //           console.log('body',body);
  //           // this.import_.update_import(body2).subscribe((data: any) => {});
  //           this.gen_qr.create_qrcode(body).subscribe((data: any) => {
  //           Swal.fire({
  //               icon: 'success',
  //               title: 'Complete!!',
  //               showConfirmButton: false,
  //               timer: 1500,
  //             }).then(function () {
  //               window.location.reload()
  //             });
  //           });
  //         } else {
  //           body.status = '1'  
  //           // this.import_.update_import(body2).subscribe((data: any) => {});    
  //           this.gen_qr.create_qrcode(body).subscribe((datas: any) => {
  //             this.gen_qr.get_qrcode_bydate().subscribe((data: any) => {
  //               if(data){
  //                 this.datasy = data
  //                 this.studentOptions3 = this.datasy.filter((item: any, i: any, arr: any) => arr.findIndex((t: any) => t.invoice === item.invoice ) === i);
  //                 console.log("this.studentOptions3",this.studentOptions3);
                  
  //                 for (let x = 0; x < this.studentOptions3.length; x++) {
  //                   for (let i = 0; i < this.datasy.length; i++) {
  //                     if(this.datasy[i].invoice == this.studentOptions3[x].invoice && this.datasy[i].model == this.studentOptions3[x].model ){
  //                       this.sum += Number(this.datasy[i].qty)      
  //                     }
  //                     console.log();
                      
  //                     // if(data[i].invoice == this.invoice){                   
  //                     //   this.data.push({ invoice: data[i].invoice, model: data[i].model, total_pallet: data[i].total_pallet, qty: this.sum, status: data[i].status })
  //                     // }
                    
  //                   this.data.push({invoice : this.studentOptions3[x].invoice,model : this.studentOptions3[x].model,total_pallet : this.studentOptions3[x].total_pallet,qty : this.sum,status : this.studentOptions3[x].status})
  //                   }
  //                   this.sum = 0
  //                   console.log("this.data",this.data);
  //                 }
                  
  //                 for (let p = 0; p < this.data.length; p++) {
  //                   if(this.data[p].invoice == this.invoice){
  //                     this.item = this.data[p]
  //                   }
  //                   console.log("this.item",this.item);
  //                 }     
  //                   this.print_label(this.item)
  //               }
  //             });
  //           });
  //         }
  //     }
  //   }
  // }
  

  // print_label(item: any) {
  //   this.count_data = []  
  //   for (let i = 0; i < this.datasy.length; i++) {  
  //     if(item.model == this.datasy[i].model && item.invoice == this.datasy[i].invoice){
  //       this.count_data.push(this.datasy[i])   
  //       let bu =    this.datasy[i].invoice + "," +  this.datasy[i].qty + "," +  this.datasy[i].no_pallet + ',' +  this.datasy[i].total_pallet
  //       this.body.push(bu)
  //     }    
  //   }
  //   for (let k = 0; k < this.count_data.length; k++) {
  //     this.count_data[k].qrInfo = this.body[k]    
  //   }
  //     this.count(item);
  //       for (let k = 0; k < this.count_data.length; k++) {
  //         this.count_data[k].qrInfo = this.body[k]    
  //       }
  //       this.count(item);
  //   // this.count(item);    
  // }


}
